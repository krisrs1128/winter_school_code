...
1. A few simple improvements to the quality of a dataset can make a much bigger
...
mention competitions), significant time is dedicated to data exploration and
...
some of the most common changes we can make to a dataset before we feed it into
a model.

2. First, a caveat: Never overwrite the raw data! Instead, write a workflow that
transforms the raw data into preprocessed data. This makes it possible to try a
few different transformations and see how the resulting performances compare.
Also, we want to have some confidence that the overall analysis is reproducible,
and if we make irreversible changes to the raw data, then we lose all chance at
that.
...
not linear in $x$, it _is_ linear in the derived features $\mathbf{1}\left\{x > 0\right\}$ and $x^2$, respectively.
