...
0). The loss increases the further we are from the truth.

8. Let's fit a linear regression in code. Below, I'm loading a dataset about
...
are $D = 10$ numeric predictors; below I print 4 samples corresponding to the first 5.
...
Let's now fit a linear model from $X$ to $y$. The first line tells python that
we are using a Linear Regression model class. The second searches over
coefficients $b$ to minimize the squared-error loss between $x^T b$ and $y$. The
third line prints out the fitted coefficient $\hat{b}$.
...
9. Let's do the same thing for a logistic regression. This time, we'll use the
...
10. It seems like we should be able to draw nice boundaries between these
...
11. The same idea can be applied to logistic regression. In this case, having a
...
12. To implement sparse linear regression using `sklearn`, we can use the
`ElasticNet` class. We'll work with the Office dataset. The task is to predict
the IMDB ratings for each episode based on which actors are in it and who the
writers were.

```{python, echo = TRUE}
office = pd.read_csv("https://uwmadison.box.com/shared/static/ab0opn7t8cagr1gj87ty94nomu5s347o.csv")
office.head()
X, y = office.iloc[:, 2:], office["imdb_rating"]
...
We can confirm that the training error is quite good, though we will discuss
later how to guard against potential overfitting.
...
17. We can implement these models in `sklearn` using `RandomForestRegressor` /
...
17. The diagram below summarizes the relationships across elements of the model
class.
...
19. Try matching models to responses in the examples below,
