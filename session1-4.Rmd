...
discussed which of the features were actually relevant. The block below refits the code we had from before.
...
12. We can visualize the CP profiles using the `plot_partial_dependence`
...
We use the `partial_dependence` function from the `sklearn.inspection` module to
compute partial dependence scores for one of the features, `bill_length_mm`.
This will give us a sense of how the three class probabilities change as we vary
the bill length, all else held equal.
